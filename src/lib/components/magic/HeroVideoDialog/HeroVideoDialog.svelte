...
<div class="relative">
	...
	<div
		class="relative cursor-pointer bg-opacity-[0.01] before:absolute before:bottom-0 before:left-0 before:top-0 before:size-full before:h-[80%] before:w-full before:opacity-0 before:[background-image:linear-gradient(to_bottom,var(--color-one),var(--color-one),transparent_40%)] before:[filter:blur(180px)] after:absolute after:bottom-0 after:left-0 after:z-0 after:h-[100%] after:w-full after:[background:linear-gradient(to_top,hsl(var(--background))_20%,transparent)]"
		on:click={openVideo}
	>
		<img
			src={thumbnailSrc}
